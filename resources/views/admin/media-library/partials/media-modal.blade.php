    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="mediaLibraryModalLabel">{{ __('admin.Media Library') }}</h5>
            ...
            <div class="modal-body">
                ...
                <ul class="nav nav-tabs" id="mediaTabs" role="tablist">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" id="library-tab" data-toggle="tab" href="#libraryTab" role="tab">
                            <i class="fas fa-images"></i> {{ __('admin.Media Library') }}
                        </a>
                    </li>
                </ul>
                ...
                <div class="tab-content mt-3" id="mediaTabsContent">
                        ...
                        <form id="quickUploadForm" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <div class="custom-file">
                                    ...
                                    <label class="custom-file-label" for="quickUploadFile">{{ __('admin.Choose file') }}</label>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="alt_text" id="quickAltText" placeholder="{{ __('admin.Enter alt text') }}">
                            </div>

                            <div class="form-group">
                                <label>{{ __('admin.Caption') }} <small class="text-muted">({{ __('admin.Optional') }})</small></label>
                                <textarea class="form-control" name="caption" id="quickCaption" rows="2" placeholder="{{ __('admin.Enter caption') }}"></textarea>
                            </div>

                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-upload"></i> {{ __('admin.Upload & Insert') }}
                            </button>
                        </form>
                    </div>
                    ...
                    <div class="tab-pane fade" id="libraryTab" role="tabpanel">
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <input type="text" id="mediaSearch" class="form-control" placeholder="{{ __('admin.Search media...') }}">
                            </div>
                            <div class="col-md-4">
                                <select id="mediaTypeFilter" class="form-control">
                                    <option value="image">{{ __('admin.Images') }}</option>
                                    <option value="all">{{ __('admin.All Types') }}</option>
                            ...
                            <div class="col-md-2">
                                <button type="button" id="loadMedia" class="btn btn-primary btn-block">
                                    <i class="fas fa-sync"></i> {{ __('admin.Refresh') }}
                                </button>
                            </div>
                        </div>
...
$(document).ready(function() {
            ...
            success: function(response) {
                if (response.success && response.media) {
                    if (selectionMode === 'featured') {
                        setFeaturedImage(response.media);
                    } else {
                        insertMediaToEditor(response.media);
                    }
    ...
    function loadMediaLibrary(page = 1) {
        ...
        $.ajax({
            ...
            error: function() {
                $('#mediaLibraryGrid').html('<div class="col-12"><div class="alert alert-danger">{{ __('admin.Error loading media') }}</div></div>');
            }
        });
    }
    ...
    function renderMediaGrid(media) {
        let html = '';
        if (media.length === 0) {
            html = '<div class="col-12"><div class="alert alert-info text-center">{{ __('admin.No media found') }}</div></div>';
        } else {
