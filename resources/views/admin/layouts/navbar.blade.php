    ...
    <ul class="navbar-nav navbar-right ml-auto">
        ...
        <li class="dropdown"><a href="#" data-toggle="dropdown"
                ...
                @php
                    ...
                    $avatarImage = !empty($admin->image) && file_exists(public_path($admin->image)) 
                        ? asset($admin->image) 
                        : asset('admin/assets/img/avatar/avatar-1.png');
                @endphp
                <img alt="image" src="{{ $avatarImage }}" class="rounded-circle mr-1">
                <div class="d-sm-none d-lg-inline-block">{{ __('admin.Hi') }}, {{ $admin->name }}</div>
            </a>
