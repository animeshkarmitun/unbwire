                ...
                <form action="{{ route('admin.category.update', $category->id) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="parent_id" id="parent_id" class="form-control select2">
                            ...
                            @foreach($parentCategories as $parent)
                                <option value="{{ $parent->id }}" data-language="{{ $parent->language }}" {{ $category->parent_id == $parent->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <select name="show_at_nav" id="" class="form-control">
                            <option {{ $category->show_at_nav == 0 || $category->show_at_nav == false ? 'selected' : '' }} value="0">{{ __('admin.No') }}</option>
                            <option {{ $category->show_at_nav == 1 || $category->show_at_nav == true ? 'selected' : '' }} value="1">{{ __('admin.Yes') }}</option>
                        </select>
                        @error('defalut')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <input name="order" type="number" class="form-control" id="order" value="{{ $category->order ?? 0 }}" min="0" placeholder="0">
